...
export class SimpleEmailService extends Service {
    ...
    private get client(): SESClient {
        if (!this._client) {
            throw new Error(
                '[SimpleEmailService] Not connected. Call connect() first.',
            );
        }
        return this._client;
    }
     ...
     * });
     */
    async sendEmail(options: SendEmailOptions): Promise<SendEmailCommandOutput> {
        const sender = options.from ?? config.SES_FROM_EMAIL;

        if (!sender) {
            throw new Error(
                '[SimpleEmailService] No sender address. Set SES_FROM_EMAIL or pass `from` in options.',
            );
        }
        ...
        if (!body.Text && !body.Html) {
            throw new Error(
                '[SimpleEmailService] At least one of `text` or `html` must be provided.',
            );
        }
