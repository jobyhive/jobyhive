...
function validateEnv(cfg) {
    ...
    if (missing.length > 0) {
        throw new Error(`[system-config] Missing required environment variables in production:\n  ${missing.join(', ')}\n` +
...
const validNodeEnvValues = ['development', 'production', 'test'];
function parseNodeEnv(value) {
    if (validNodeEnvValues.includes(value)) {
        return value;
    }
    return 'development';
}
